...
public class FindImages : ScriptBase
{
    ...
    void Execute() 
    {    
        Log(Path.GetFullPath(ImageFinder.FindImageDllPath));
        Log(SourceDllpath);
        
        CreateEvent("SourceDllpath", $"{SourceDllpath}");
        CreateEvent("CurrentUserTempDir", $"{SourceDllpath}");
        CreateEvent("FindImageDLLPath", $"{Path.GetFullPath(System.IO.Path.GetTempPath())}");
